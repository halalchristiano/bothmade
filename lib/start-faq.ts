 ...
 *
 * This lives outside the page component because two different worlds need
 * it: the page renders it in the browser, and app/start/layout.tsx — a
 * server component — turns it into FAQPage structured data. A server
 * component that imports a value from a `'use client'` module receives a
 * client-reference stub rather than the value, so reading the array
 * straight from the page silently yields something that isn't an array.
 * One shared, environment-neutral module is what makes both work, and
 * keeps the markup from ever drifting from the copy on screen.
 */
