  ...
  return [
    ...
    },
    {
      ...
      paragraphs: [
        ...
        `A deposit of ${p.depositAmount} (${p.depositPercent}% of the Total Fee) is due before work begins. The remaining balance of ${p.balanceAmount} is due upon completion of the Build phase and prior to Launch, unless the Parties agree to a different schedule in writing (see Exhibit B). Where the Client cancels after the cancellation window in Section 8(b) has closed, that balance accelerates and becomes immediately due under Section 8(e) instead.`,
    ...
    },
    {
      ...
      paragraphs: [
        ...
        `(e) Cancellation on or after the ${CANCELLATION_WINDOW_DAYS}th day. Beginning on the fourteenth (${CANCELLATION_WINDOW_DAYS}th) calendar day after the Project Start Date, the initial ${p.depositPercent}% payment of ${p.depositAmount} is fully earned and non-refundable. The remaining ${100 - p.depositPercent}% of ${p.balanceAmount} becomes immediately due and payable on the date written notice of cancellation is given, and is payable within fifteen (15) Business Days of that date. This acceleration operates notwithstanding Section 7 and Exhibit B: the balance does not wait on completion of Build, on Launch, or on any other Milestone, condition, or further performance by the Agency, none of which can occur on a Project the Client has cancelled. The Client's total liability under this paragraph is the Total Fee of ${p.totalPrice} and no more.`,
        `(f) What the Client receives on acceleration. Paragraph (e) is not a charge for nothing. On payment of the accelerated balance the Client receives every Deliverable and work product produced to the date of cancellation, in its then-current state, together with the source, credentials, and documentation within scope, and the assignment of intellectual property under Section 12 takes effect in full as though the Project had completed. The Agency has no obligation to perform further work after cancellation, and may at its sole option complete outstanding work without additional charge.`,
        `(g) Acknowledgment as to acceleration. The Parties acknowledge that the Agency prices and schedules an engagement as a whole: on accepting this Project the Agency reserves capacity for its full duration, declines or defers other engagements it could otherwise have taken, and cannot resell that capacity at short notice when a Project is cancelled part-way through. The Parties therefore agree that the Total Fee is a genuine pre-estimate, agreed in advance, of the Agency's loss on a cancellation after the ${CANCELLATION_WINDOW_DAYS}-day window, that it protects a legitimate business interest in the whole of the bargain rather than punishing the Client for cancelling, that the ${CANCELLATION_WINDOW_DAYS}-day window in (b) exists precisely so the Client has a real and unpenalised opportunity to withdraw before that exposure attaches, and that the Fees were set on the basis of this allocation of risk. The Client acknowledges it had the opportunity to negotiate this term and to take independent legal advice before accepting it.`,
        '(h) Scope of this policy. Paragraphs (b) through (g) apply to cancellation by the Client for convenience, and do not eliminate any rights or remedies that cannot legally be waived.',
        '(i) Where the Agency is the one leaving. Where instead the Agency terminates for convenience under Section 11, or the Client terminates because of the Agency\'s uncured material breach or its default under (k) below, no deduction under (b)(1) applies: the Agency retains only the value of time worked at the Cancellation Rate plus nonrecoverable expenses, and refunds the balance of everything paid.',
        '(j) Missed timeline estimates: a timeline running longer than projected does NOT, on its own, entitle the Client to a refund, provided the Agency continues to work in good faith and has not abandoned the Project — particularly where delay is caused or contributed to by Client Dependencies (Section 6), scope changes (Section 9), or third-party dependencies (Section 3).',
        '(k) Agency-caused unreasonable delay: where the Agency ceases substantive work for more than forty-five (45) consecutive days without a Client-caused reason and without reasonable communication, the Client may issue written notice of default. If the Agency does not resume work or provide a remediation plan within fifteen (15) Business Days, the Client may terminate and be refunded on the basis set out in (i), whether or not the cancellation window in (b) has closed.',
        '(l) Completed and accepted Deliverables are not refundable except for the Agency\'s gross negligence or willful misconduct in producing that specific Deliverable.',
        '(m) Chargebacks: initiating a card chargeback in lieu of the process in this Section is a material breach and may result in immediate suspension of Deliverables, Preview Environments, and dashboard access; the Agency may contest any chargeback with evidence of work performed.',
        '(n) A refund due under this Section is processed to the original payment method within fifteen (15) Business Days, less any non-recoverable processor fees. An amount due from the Client under (e) is payable within fifteen (15) Business Days of the Agency\'s written statement of the amount.',
        '(o) This Refund Policy is the Client\'s sole and exclusive remedy for dissatisfaction with the pace, quality, or outcome of the Project, except where a separate remedy is expressly provided elsewhere in this Agreement or required by non-waivable consumer protection law.',
      ],
    },
    {
    ...
    },
    {
      ...
      paragraphs: [
        ...
        'Where a delay is solely and directly attributable to the Agency\'s own scheduling (and none of the causes above), the Client\'s sole remedy is set out in Section 8(k) — the Client is not entitled to consequential damages, lost profits, or the cost of a replacement vendor, subject to Section 15 (Limitation of Liability).',
      ],
    },
    {
      heading: '11. Termination',
      paragraphs: [
        'Either Party may terminate for convenience on thirty (30) days\' written notice. The financial consequence is fixed entirely by Section 8 — paragraphs (b) to (g) where the Client terminates, and paragraph (i) where the Agency does — and no other cancellation charge, fee, or damages arise from the termination itself.',
    ...
    },
    {
      ...
      paragraphs: [
        ...
        `If the Client cancels within ${CANCELLATION_WINDOW_DAYS} calendar days of the Project Start Date, the refund is what was paid less the greater of ${cancellationFloor(
          p
        )} (${CANCELLATION_FLOOR_PERCENT}% of the Total Fee) or time worked at the Cancellation Rate plus nonrecoverable expenses. On or after the ${CANCELLATION_WINDOW_DAYS}th day, the ${p.depositAmount} paid up front is fully earned and the ${p.balanceAmount} balance becomes immediately due, whatever stage the work had reached — the Client receives everything produced to that date, with IP assigned in full. Section 8 controls if this summary and it ever diverge.`,
      ],
    },
    {
      ...
      paragraphs: [
        ...
        `Worked example, cancelled after the window: the Client cancels on day 20. The ${p.depositAmount} paid up front is fully earned and not refundable, and the remaining ${p.balanceAmount} becomes immediately due — payable within 15 Business Days, not on completion of Build, which will now never happen. The Client pays the full ${p.totalPrice} and receives every Deliverable produced to that date with intellectual property assigned in full, as though the Project had completed. Hours worked are not recalculated either way.`,
        ...
        'The Agency goes quiet for over 45 days with no explanation: the Client may issue a written default notice under Section 8(k); if the Agency doesn\'t resume or provide a plan within 15 Business Days, the Client can terminate and be refunded on the Section 8(i) basis — the Agency keeps only the value of work actually performed and accepted, with no cancellation element.',
        ...
        'The Client files a card chargeback instead of requesting a refund: this is treated as a breach separate from the underlying Fee dispute under Section 8(m), and the Agency may suspend access while contesting it with evidence of work performed.',
      ],
    },
  ];
