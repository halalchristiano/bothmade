...
const already = new Set(
  ['harvest.tsv','harvest2.tsv'].flatMap((f) =>
...
const mailed = new Set(
  readFileSync('harvest2.tsv','utf8').split('\n').filter(Boolean)
    .map((l) => (l.split('\t')[1]||'').trim().toLowerCase()).filter(Boolean)
);

const rows = readFileSync('harvest3.tsv','utf8').split('\n').filter(Boolean).map((l) => {
...
for (const r of rows) {
  ...
  out.push({
    ...
    tags: `field-run-005;${r.niche.toLowerCase().replace(/\s+/g,'-')};${r.state};${r.verified}-email`,
