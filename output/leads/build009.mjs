...
const already = new Set(
  ['harvest.tsv','harvest2.tsv','harvest3.tsv','harvest4.tsv','harvest5.tsv','harvest6.tsv'].flatMap((f) =>
...
const mailed = new Set(
  ['harvest2.tsv','harvest3.tsv','harvest4.tsv','harvest5.tsv','harvest6.tsv'].flatMap((f) =>
    readFileSync(f,'utf8').split('\n').filter(Boolean)
...
for (const r of rows) {
  ...
  out.push({
    ...
    tags: `field-run-009;${r.niche.toLowerCase().replace(/\s+/g,'-')};${r.state};${r.verified}-email`,
