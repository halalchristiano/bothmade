...
const already = new Set(
  ['harvest.tsv','harvest2.tsv','harvest3.tsv','harvest4.tsv','harvest5.tsv'].flatMap((f) =>
...
const mailed = new Set(
  ['harvest2.tsv','harvest3.tsv','harvest4.tsv','harvest5.tsv'].flatMap((f) =>
    readFileSync(f,'utf8').split('\n').filter(Boolean)
      .map((l) => (l.split('\t')[1]||'').trim().toLowerCase())
  ).filter(Boolean)
);

const rows = readFileSync('harvest6.tsv','utf8').split('\n').filter(Boolean).map((l) => {
...
for (const r of rows) {
  ...
  out.push({
    ...
    tags: `field-run-008;${r.niche.toLowerCase().replace(/\s+/g,'-')};${r.state};${r.verified}-email`,
