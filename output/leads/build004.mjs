...
const already = new Set(
  readFileSync('harvest.tsv','utf8').split('\n').filter(Boolean)
    .map((l) => l.split('\t')[0].toLowerCase().replace(/[^a-z0-9]/g,''))
);

const rows = readFileSync('harvest2.tsv','utf8').split('\n').filter(Boolean).map((l) => {
...
for (const r of rows) {
  ...
  out.push({
    ...
    tags: `field-run-004;${r.niche.toLowerCase().replace(/\s+/g,'-')};${r.state};${r.verified}-email`,
