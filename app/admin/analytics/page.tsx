 ...
 */
function RevenueChart({ data }: { data: Analytics['revenue']['byMonth'] }) {
        ...
        {data.map((month, i) => {
          ...
          return (
            <div
              ...
              title={`${month.label}: ${formatCents(month.oneOff)} project work + ${formatCents(month.recurring)} care plans`}
            >
              <span className="-mb-1 text-[10px] font-medium text-white/25 transition-colors group-hover:text-white/60">
              ...
              <div className="flex h-24 w-full flex-col justify-end">
                <div
                  className="w-full rounded-t-sm bg-emerald-400/60 transition-colors group-hover:bg-emerald-400/80"
                  style={{ height: bar(month.recurring) }}
                />
                <div
                  className={`w-full bg-sky-400/45 transition-colors group-hover:bg-sky-400/75 ${
                    month.recurring > 0 ? '' : 'rounded-t-sm'
                  }`}
                  style={{ height: bar(month.oneOff) }}
                />
              </div>
              <span className="text-[10px] text-white/30">{month.label}</span>
            </div>
          );
        })}
      </div>
