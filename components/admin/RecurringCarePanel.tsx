 ...
 *
 * The panel is built around the discount being a decision made here, per
 * client, and the schedule being visible before anything is sent. A rate
 * that's introductory only if somebody remembers to change it later isn't a
 * discount, it's a mistake with a delay on it — so the preview always shows
 * all three phases, including the month the standard rate resumes.
 *
...
export function RecurringCarePanel({ projectId }: { projectId: string }) {
  ...
  if (loading) {
    return (
      <div className="rounded-2xl border border-white/[0.08] bg-gradient-to-b from-white/[0.06] to-white/[0.02] backdrop-blur-xl p-6">
        <p className="text-sm text-white/40">Loading care plans…</p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl border border-white/[0.08] bg-gradient-to-b from-white/[0.06] to-white/[0.02] backdrop-blur-xl p-6">
      <h2 className="text-lg font-bold mb-1">Monthly Care Plan</h2>
